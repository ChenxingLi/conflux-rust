...
impl BlockGenerator {
    ...
    pub fn generate_custom_block(
        ...
        let maybe_base_price = self.txpool.compute_1559_base_price(&parent_hash, (GENESIS_GAS_LIMIT*ELASTICITY_MULTIPLIER as u64).into(), transactions.iter().map(|x|&**x)).unwrap();
        let block_gas_limit = GENESIS_GAS_LIMIT * if maybe_base_price.is_some() {
