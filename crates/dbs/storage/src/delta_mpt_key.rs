...
use primitives::{
    MerkleHash, StorageKey, StorageKeyWithSpace, MERKLE_NULL_NODE,
    NO_ACCOUNT_LENGTH_CHECK,
};
...
impl Deref for DeltaMptKeyPadding {
    type Target = [u8; KEY_PADDING_BYTES];
...
const _: () = assert!(
    ACCOUNT_KEYPART_BYTES == primitives::DELTA_MPT_ACCOUNT_KEYPART_BYTES
);
...
pub fn storage_key_from_delta_mpt_key<'a>(
    delta_mpt_key: &'a [u8],
    ...
    if bytes_len < ACCOUNT_KEYPART_BYTES {
        ...
        let address_bytes =
            &remaining_bytes[ACCOUNT_PADDING_BYTES..ACCOUNT_KEYPART_BYTES];
        if bytes_len == ACCOUNT_KEYPART_BYTES {
        ...
        {
            let bytes =
                &remaining_bytes[StorageKeyWithSpace::CODE_HASH_PREFIX_LEN..];
            if !bytes.is_empty() {
                StorageKey::CodeKey {
                    address_bytes,
...
pub fn to_delta_mpt_key_bytes(
    key: &StorageKeyWithSpace, padding: &DeltaMptKeyPadding,
) -> Vec<u8> {
    let key_bytes = match key.key {
        StorageKey::AccountKey(address_bytes) => {
            if address_bytes.len() == StorageKeyWithSpace::ACCOUNT_BYTES {
                new_account_key(address_bytes, padding)
            } else if NO_ACCOUNT_LENGTH_CHECK {
                // The branch is test only. When an address with incomplete
                // length, it's passed to DeltaMPT directly.
        ...
        StorageKey::StorageRootKey(address_bytes) => {
            new_storage_root_key(address_bytes, padding)
        }
        StorageKey::StorageKey {
            address_bytes,
            storage_key,
        } => new_storage_key(address_bytes, storage_key, padding),
        StorageKey::CodeRootKey(address_bytes) => {
            new_code_root_key(address_bytes, padding)
        }
        StorageKey::CodeKey {
            address_bytes,
            code_hash_bytes,
        } => new_code_key(address_bytes, code_hash_bytes, padding),
        StorageKey::DepositListKey(address_bytes) => {
            new_deposit_list_key(address_bytes, padding)
        }
        StorageKey::VoteListKey(address_bytes) => {
            new_vote_list_key(address_bytes, padding)
        }
        StorageKey::EmptyKey => {
            return vec![];
        }
...
mod tests {
    use super::{
        ...
        DeltaMptKeyPadding, KEY_PADDING_BYTES,
    };
    ...
    fn test_delta_mpt_account_key() {
        ...
        let bytes = to_delta_mpt_key_bytes(&key, &padding);
        let key2 = storage_key_from_delta_mpt_key(&bytes[..]);
        assert_eq!(key, key2);
