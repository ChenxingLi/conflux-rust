...
fn test_empty_genesis_block() {
    ...
    state_manager
        .get_state_trees(&genesis_epoch_id, /* try_open = */ false, true)
        .unwrap();
}
...
fn test_get_set_at_second_commit() {
    ...
    let mut state_1 = state_manager
        .clone()
        .open_next_epoch_base(&epoch_id_0, OpenOptions::new())
        .unwrap()
        .unwrap();
...
fn test_set_order_concurrent() {
    ...
    let mut state_1 = state_manager
        .clone()
        .open_next_epoch_base(&parent_epoch_0, OpenOptions::new())
        .unwrap()
        .unwrap();
    println!("Setting state_1 with {} keys.", keys.len());
    for key in keys.iter() {
