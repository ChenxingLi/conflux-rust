...
impl State {
    ...
    pub(crate) fn commit(
        ...
        // Whether a snapshot generated by this commit has to rebuild its MPT
        // from scratch. Asked on every commit, not only on the ones which do
        // make a snapshot, because the same call is what ends the recovery
        // mode once the commit height reaches the end of the replay window.
