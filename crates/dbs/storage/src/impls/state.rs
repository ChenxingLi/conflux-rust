...
impl State {
    ...
    /// The commit write port of the private index, next to
    /// `StateManager::migrate_state_index` and
    /// `StorageManager::register_synced_snapshot_state`.
...
// tests the only caller is `WritableState`.
impl State {
    pub(crate) fn set(
    ...
    pub(crate) fn commit(
        ...
        // entries anyway. A missing entry makes `usable_as_base` answer no, so
        // the recomputation start falls back to somewhere near the newest
        // snapshot and every commit on the way up registers its entry again.
        // The hole heals within the same startup, with no repair pass.
        self.write_state_index_entry(&epoch_id, &merkle_root)?;

        // Called on every commit, not just snapshot ones: the same call ends
        // recovery mode when the commit height reaches the replay window end.
        let recover_mpt_during_construct_pivot_state = self
            .manager
            .get_storage_manager()
            .recover_mpt_for_commit(self.height);
...
impl State {
    ...
    /// Nothing reaches the db. `Drop` panics on a dirty state, so `revert`
    /// must run before the object is dropped. `revert` returns every allocated
    /// node to the delta MPT's allocator, so a later pass over the same base
...
impl State {
    ...
    /// Retrieve all key/value pairs with access_key_prefix as prefix. These
    ...
    fn read_all_impl(
        ...
        // `SubTrieVisitor` wants a mutable borrow of an owned node set because
        // the modifying visitors register the nodes they take ownership of in
        // it. The read-only traversal below never registers anything: it only
        // reads the set, to decide the `owned` flag of the temporary
        // `CowNodeRef`s it creates and drops along the way, and that flag is
        // never consulted on this path. A local empty set is therefore enough,
        // and it lets the whole prefix lookup run on `&self`.
        let mut owned_node_set = Some(OwnedNodeSet::default());

        // Retrieve key/value pairs from delta trie
